...
if not luAnimation then
  sb.logInfo("LuAnimator: Animation script could not be loaded at '/tech/blink/luanimation.lua'!")
end
...
-- Disables keybinds defined in luAnimator.keybinds from being activated.
function luAnimator.lockInput()
  luAnimator.inputLocked = true
end

-- Unlocks keybinds, locked by luAnimator.lockInput()
function luAnimator.unlockInput()
  luAnimator.inputLocked = false
end

-- Returns the current state, based on the position of the player's entity.
...
-- Returns a value indicating whether the player's entity is in a liquid or not.
function luAnimator.isInLiquid()
  return world.liquidAt(mcontroller.position()) ~= nil
end

-- Returns a value indicating whether the player's entity is on the ground or not.
...
-- Resets the sit offset to it's default value; around one character high.
function luAnimator.resetSitOffset()
  luAnimator.sitOffset = { x = 0, y = 3.5 }
end

-- Runs code every update that handles sitting on entities.
...
-- Runs code every update that handles animating.
...
-- Unitialize function, called when leaving instances or quitting the game.
